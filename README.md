...
| `/face-scripts <логины>` | Face ID: zip-архив с фото и Excel-таблица по списку логинов |

### `/face-scripts`

Принимает список логинов через пробел, запятую или с новой строки:

```
/face-scripts ivan petr olga
```

Для каждого логина бот запрашивает у 01-edu данные пользователя
(`GetUserByLogin` → `lastNameCyr`, `firstNameCyr`, `IIN`, `photoUploadId`) и
скачивает фото через `/api/storage`. В ответ приходят два файла:

- **Excel-таблица** (`faceid_<дата>.xlsx`) со столбцами `№, Lastname,
  Firstname, Photo, IIN/ID`; фото встроены прямо в ячейки.
- **ZIP-архив** (`faceid_photos_<дата>.zip`) с фото в полном разрешении.

Ненайденные логины и ошибки не прерывают обработку — они помечаются в таблице,
а в подписи к файлу приводится сводка. За один вызов обрабатывается не более
200 логинов.
